...
# Import Excel validator (ถ้ามี)
try:
    from excel_validator import is_valid_excel, repair_excel_file
    EXCEL_VALIDATOR_AVAILABLE = True
except ImportError:
    EXCEL_VALIDATOR_AVAILABLE = False
    ...
    def profile(self, max_sample_rows: int = 10000) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.file_extension in ['.csv', '.txt']:
                ...
                # ถ้าเป็นไฟล์ Excel ให้ตรวจสอบความถูกต้องของไฟล์ก่อน (ถ้ามี excel_validator)
                if EXCEL_VALIDATOR_AVAILABLE:
                    is_valid, error_message = is_valid_excel(self.file_path)
                    if not is_valid:
                        print(f"Invalid Excel file: {error_message}")
                        # ลองซ่อมแซมไฟล์
                        repaired, repair_result = repair_excel_file(self.file_path)
                        if repaired:
                            print(f"Successfully repaired Excel file: {repair_result}")
                            # ใช้ไฟล์ที่ซ่อมแซมแล้ว
                            self.file_path = repair_result
                        else:
                            # ถ้าซ่อมแซมไม่สำเร็จ ให้ลองเป็น CSV แทน
                            print(f"Failed to repair Excel file, trying as CSV: {repair_result}")
                            try:
                                # ลองอ่านเป็น CSV
                                ...
                                print(f"Failed to process as CSV: {csv_e}")
                                return {"error": f"Failed to load Excel file: {error_message}. Cannot process as CSV either."}
                
                # ไม่ว่าจะมี validator หรือไม่ ให้ลองอ่านด้วย _load_excel_data
                try:
                    df = self._load_excel_data(max_sample_rows)
                    # ตรวจสอบว่าได้ข้อมูลมาหรือไม่
                    if df is None or df.shape[0] == 0:
                        # ถ้าไม่มีข้อมูล ให้สร้าง DataFrame พื้นฐาน
                        df = pl.DataFrame({
                            "Column1": ["Excel file could not be processed"],
                            "Information": ["No data found or file is corrupted"]
                        })
                except Exception as e:
                    print(f"Excel loading failed with exception: {e}")
                    print(traceback.format_exc())  # พิมพ์ stacktrace
    ...
    def _load_excel_data(self, max_rows: int) -> pl.DataFrame:
        ...
        try:
            ...
            if file_size == 0:
                ...
                return pl.DataFrame({"Empty_File": ["This Excel file contains no data"]})
            
            # ตรวจสอบเบื้องต้นว่าไฟล์อาจเป็นข้อความล้วนๆ
            with open(self.file_path, 'rb') as f:
                header = f.read(16)
                
            # ถ้าไฟล์ดูเหมือนเป็นข้อความล้วนๆ (ไม่มี binary markers)
            if all(b < 128 and b not in [0] for b in header):
                print("File appears to be plain text, not Excel binary")
                
                # ลองอ่านเป็นข้อความ
                with open(self.file_path, 'r', encoding=self.encoding, errors='replace') as f:
                    content = f.read(1024)  # อ่านตัวอย่าง
                
                # ถ้าเป็นข้อความที่มีแค่บรรทัดว่าง
                if not content.strip():
                    print("File contains only whitespace")
                    return pl.DataFrame({"Empty_File": ["This file contains only whitespace"]})
                
                # ถ้ามีลักษณะของ CSV
                if ',' in content or '\t' in content or ';' in content:
                    print("File appears to be CSV/TSV despite extension")
                    # ใช้ delimiter ที่พบมากที่สุด
                    delimiter_counts = {
                        ',': content.count(','),
                        '\t': content.count('\t'),
                        ';': content.count(';')
                    ...
                    # เรียกใช้ _load_csv_data แทน
                    return self._load_csv_data(max_rows)
        except Exception as e:
            print(f"Error during initial Excel file check: {e}")
        
        # ลองหลายวิธีสำหรับการอ่านไฟล์ Excel
        for attempt, method in enumerate([
            "pandas_default",  # ไม่ระบุ engine
            "openpyxl",        # สำหรับ .xlsx
            "xlrd",            # สำหรับ .xls
            "odf",             # สำหรับไฟล์ที่อาจถูกบันทึกในรูปแบบ ODF
            "pyxlsb",          # สำหรับไฟล์ .xlsb
            "manual_parsing"   # วิธีสุดท้าย - ลองอ่านเป็นข้อความธรรมดา
        ]):
            try:
                print(f"Trying Excel loading method #{attempt+1}: {method}")
                
                if method == "pandas_default":
                    ...
                    # วิธีที่ 2: ระบุ engine เป็น openpyxl สำหรับ .xlsx
                    df_pd = pd.read_excel(
                        self.file_path,
                        nrows=max_rows,
                        header=0 if self.header else None,
                    ...
                    # วิธีที่ 3: ระบุ engine เป็น xlrd สำหรับ .xls
                    df_pd = pd.read_excel(
                        self.file_path,
                        nrows=max_rows,
                        header=0 if self.header else None,
                    ...
                    # วิธีที่ 5: ลองใช้ pyxlsb สำหรับไฟล์ .xlsb
                    df_pd = pd.read_excel(
                        self.file_path,
                        nrows=max_rows,
                        header=0 if self.header else None,
                    ...
                    # วิธีที่ 6: อ่านเป็นข้อความและแปลงเอง
                    print("Trying to read file as text")
                    with open(self.file_path, 'r', encoding=self.encoding, errors='replace') as f:
                    ...
                    if not lines or all(not line for line in lines):
                        ...
                        return pl.DataFrame({"Result": ["File contains no readable data"]})
                    
                    # ลองหา delimiter ที่เหมาะสม
                    potential_delimiters = [',', '\t', ';', '|']
                    delimiter_counts = {}
                    
                    for d in potential_delimiters:
                        delimiter_counts[d] = sum(line.count(d) for line in lines[:10])
                    
                    if max(delimiter_counts.values(), default=0) > 0:
                        best_delimiter = max(delimiter_counts, key=delimiter_counts.get)
                        
                        # แยกข้อมูลตาม delimiter
                        data = [line.split(best_delimiter) for line in lines]
                        headers = data[0] if self.header else [f"Column_{i+1}" for i in range(len(data[0]))]
                        rows = data[1:] if self.header else data
                        
                        # สร้าง DataFrame
                        df_dict = {}
                        for i, header in enumerate(headers):
                            df_dict[header] = [row[i] if i < len(row) else None for row in rows]
                        
                        df_pd = pd.DataFrame(df_dict)
                    else:
                        # ถ้าไม่พบ delimiter ที่ชัดเจน ให้สร้าง DataFrame ด้วยข้อมูลที่มี
                        df_pd = pd.DataFrame({"Content": lines})
                
                print(f"Successfully loaded Excel with {method}, shape: {df_pd.shape}")
                ...
                # บันทึก exception เพื่อการแก้ไขปัญหา
                traceback.print_exc()
        
        # ถ้าทั้งหมดล้มเหลว ให้สร้าง DataFrame ที่มีข้อความเตือน
        print("All Excel loading methods failed")
        return pl.DataFrame({
            "Error": ["Could not load Excel file"],
            "Message": ["File format might be corrupted or unsupported"]
        })
